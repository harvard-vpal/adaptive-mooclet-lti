...
def test_get_explanation_for_student (question_id):
	params = { "question_id": question_id, "student_id": 1 }
	encodedParams = urllib.urlencode(params)
...
def test_get_explanations_for_question (question_id):
	result = json.loads(urllib2.urlopen("http://localhost:8000/adaptive_engine_app/get_explanations_for_question?question_id={}".format(question_id)).read())
	print result
...
def test_get_question (question_id):
	result = json.loads(urllib2.urlopen("http://localhost:8000/adaptive_engine_app/get_question?id={}".format(question_id)).read())
	print result

def test_remove_question (question_id):
	result = json.loads(urllib2.urlopen("http://localhost:8000/adaptive_engine_app/remove_question?id={}".format(question_id)).read())
	print result
